...
impl<TaskType: 'static + Task + Send> WorkQueue<TaskType> {
    pub fn new(n_workers: usize) -> WorkQueue<TaskType> {
        ...
        //unimplemented!()
        let (sender1, receiver1) = spmc::channel::<TaskType>();
        let (sender2, receiver2) = mpsc::channel::<TaskType::Output>();
        let mut workers = vec![];

        for _ in 0..n_workers {
            let recv_tasks = receiver1.clone();
            let send_output = sender2.clone();
        ...
        WorkQueue {
            send_tasks: Some(sender1),
            recv_tasks: receiver1,
            recv_output: receiver2,
            workers,
        }
    }
    ...
    pub fn enqueue(&mut self, t: TaskType) -> Result<(), spmc::SendError<TaskType>> {
        ...
        //unimplemented!()
        let sender_tasks = self.send_tasks.as_mut().unwrap();
        sender_tasks.send(t)
    }
    ...
    pub fn shutdown(&mut self) {
        ...
        self.workers.drain(..).for_each(|th| {
            th.join().unwrap();
        });
    }
}
